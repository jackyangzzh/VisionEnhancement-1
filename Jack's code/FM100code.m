...
% pathrefs = cd;
% path = pwd;
% file = './Images/Images_Nov/164/capture/164.raw';
% header_file = './Images/Images_Nov/164/capture/164.hdr';
% whiteFile = './Images/Images_Nov/162/capture/162.raw';
% darkForWhiteFile = './Images/Images_Nov/164/capture/DARKREF_164.raw'; %dark reference taken at same exposure time as white
% darkForSampleFile = './Images/Images_Nov/164/capture/DARKREF_164.raw'; %dark reference taken at same exposure time as sample
...
pathrefs = cd;
path = pwd;
file = './FM-100_Hue_Test_Hyperspectral_images/023/capture/023.raw';
header_file = './FM-100_Hue_Test_Hyperspectral_images/023/capture/023.hdr';
whiteFile = './FM-100_Hue_Test_Hyperspectral_images/023/capture/023.raw';
darkForWhiteFile = './FM-100_Hue_Test_Hyperspectral_images/023/capture/DARKREF_023.raw'; %dark reference taken at same exposure time as white
darkForSampleFile = './FM-100_Hue_Test_Hyperspectral_images/023/capture/DARKREF_023.raw'; %dark reference taken at same exposure time as sample
[wavelengths, spatial, frames, spectral, tint, settings ] = parseHdrInfo_IQ(path,header_file); %Extract wavelength [nm] and other params.
...
x_length = size(S_data,1);
y_length = size(S_data,2);
S_mean = mean(reshape(S_mat, [x_length*y_length,204]),1);
S_mean = S_mean(1:147);
...
lab_data = xyz2lab(XYZ,'WhitePoint','d65');
toc
